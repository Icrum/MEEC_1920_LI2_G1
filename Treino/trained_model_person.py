    ...
    for i, (dirpath, dirnames, filenames) in enumerate(os.walk(dataset_path)):
        ...
        for f in filenames:
            ...
            while True:
                ...
                mfcc = np.mean(
                    librosa.feature.mfcc(data_shift, sr=SAMPLE_RATE, n_mfcc=num_mfcc, n_fft=n_fft, hop_length=hop_length).T,
                    axis=0)
                X.append(mfcc)
                y.append(i - 1)
                ...
                # mfcc = np.mean(
                #     librosa.feature.mfcc(data_stretch, sr=SAMPLE_RATE, n_mfcc=num_mfcc, n_fft=n_fft, hop_length=hop_length).T,
                #     axis=0)
                # X.append(mfcc)
                # y.append(i - 1)
...
if __name__ == "__main__":
    ...
    sc_filename = 'sc_model_person.bin'
    ...
    filename = 'trained_model_person.bin'
